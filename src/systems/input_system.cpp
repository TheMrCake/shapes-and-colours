...
void InputSystem::update(const float delta_time) {
  ComponentMap<Input>& input_component_map = entity_manager.get_component_map<Input>();  
  for (auto& [entity_id, input_component] : input_component_map) {
    ...
    input_component->input_direction = Box2DUtils::normalize(input_component->input_direction);
  }
}
