...
GameScene::GameScene(sf::Vector2u windowSize, EntityManager &em)
    ...
    bulbSprite.setOrigin(bulbTexture.getSize().x / 2.f,
                         bulbTexture.getSize().y / 2.f);
    ...
    // Create light beam
    beamId =
        create_light_ray(entityManager, windowSize, sf::Vector2f(-1.f, 1.f));
    ...
    if (event.type == sf::Event::KeyPressed &&
        ...
        if (!paused) {
            ...
            for (auto &[id, lightPtr] : lights) {
                if (auto shapeOpt =
                        entityManager.get_entity_component<Shape>(id)) {
...
void GameScene::render(sf::RenderWindow &window) {
    ...
    if (frameCount++ % 60 == 0) {
        std::cout << "Rendering " << visibleCount << " visible shapes out of "
                  << shapes.size() << " total\n";
    }
}
