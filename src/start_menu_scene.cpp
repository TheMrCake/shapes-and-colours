...
StartMenuScene::StartMenuScene(sf::Vector2u windowSize, EntityManager &em)
    ...
    m_title.setPosition(windowSize.x / 2.f -
    ...
    m_startText.setPosition(windowSize.x / 2.f -
    ...
    m_quitText.setPosition(windowSize.x / 2.f -
    ...
    if (event.type == sf::Event::KeyPressed) {
        if (event.key.code == sf::Keyboard::Up ||
            ...
            if (m_selectedIndex == 0) {
                m_next = new GameScene(m_windowSize,
                                       entityManager); // start gameplay
            } else {
                m_next = nullptr; // exit
            }
        }
